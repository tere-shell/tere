...
enum MakePtyError {
    ...
    #[error("cannot make child PTY non-blocking: {0}")]
    MakeAsync(#[source] std::io::Error),
}
...
fn make_pty() -> Result<(PtyMaster, Async<std::fs::File>), MakePtyError> {
    ...
    let pty_child = Async::new(pty_child).map_err(MakePtyError::MakeAsync)?;
...
async fn pty_io() {
    ...
    let client_task = smol::spawn(async move {
        ...
        }
        {
            ...
            conn.send_with_fds(&msg).await.expect("send Request");
        }
